...
test.describe('Gamification Pages', () => {
  test('成就页面加载并显示筛选按钮', async ({ page }) => {
    ...
    // Category filter buttons are always rendered when data loads
    await expect(page.getByRole('button', { name: '全部' })).toBeVisible({ timeout: 5000 });
    await expect(page.getByRole('button', { name: '竞赛' })).toBeVisible({ timeout: 5000 });
  });
  ...
  test('排行榜页面加载并显示标签页', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: '好友排行' })).toBeVisible({ timeout: 5000 });
    await expect(page.getByRole('button', { name: '地区排行' })).toBeVisible({ timeout: 5000 });
  });
  ...
  test('虚拟商店页面加载并显示标签页', async ({ page }) => {
    ...
    await expect(page.locator('button').filter({ hasText: '称号' })).toBeVisible({ timeout: 5000 });
    await expect(page.locator('button').filter({ hasText: '头像框' })).toBeVisible({ timeout: 5000 });
  });
